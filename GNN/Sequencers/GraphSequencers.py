...
class MultiGraphSequencer(tf.keras.utils.Sequence):
    ...
    def __init__(self,
        ...
        self.indices = self.build_indices() #np.arange(len(self.data))
        self.focus = focus
    ...
    @property
    def targets(self):
        return np.concatenate([g.targets if self.focus=='g' else g.targets[g.set_mask[g.output_mask]] for g in self.batches], axis=0)
    ...
    def aggregation_mode(self, _aggregation_mode):
        for g in self.data: g.aggregation_mode = _aggregation_mode
        self._aggregation_mode = _aggregation_mode
...
class SingleGraphSequencer(MultiGraphSequencer):
    ...
    def __init__(self,
        ...
        self.data = self.to_graph_tensor(graph)

    def build_indices(self):
        return np.argwhere(self.data[0].set_mask).squeeze()
    ...
    @property
    def targets(self):
        return self.data.targets if self.focus=='g' else self.data.targets[self.data.set_mask[self.data.output_mask]]
...
class CompositeSingleGraphSequencer(SingleGraphSequencer, CompositeMultiGraphSequencer):
    ...
    # Specific function utilities.
    to_graph_tensor = classmethod(CompositeGraphTensor.fromGraphObject)
    build_build_indices = SingleGraphSequencer.build_indices
