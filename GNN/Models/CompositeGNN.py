...
class CompositeGNNnodeBased(tf.keras.Model):
    ...
    # -----------------------------------------------------------------------------------------------------------------
    def apply_filters(self, state_converged, nodes, adjacency, arcs_label, mask,dim_node_features) -> tf.Tensor:
        """ Takes only nodes' states for those with output_mask==1 AND belonging to set. """
        state_converged = tf.concat([state_converged, nodes[:, :dim_node_features[0]]], axis=1)
    ...
    def Loop(self, nodes, arcs, dim_node_features, type_mask, set_mask, output_mask, composite_adjacencies, adjacency,
        ...
        input_to_net_output = self.apply_filters(state, nodes, adjacency, arcs, mask, dim_node_features)
