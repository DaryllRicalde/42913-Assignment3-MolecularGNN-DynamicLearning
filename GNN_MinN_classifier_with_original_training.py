...
def custom_fit(model, train_dataset , epochs, validation_data, callbacks):
    model.compile()
    for epoch in range(epochs):
        ...
        # print("Validation data shape", np.shape(validation_data)) # there is only one graph
        # print("batch num: ", i)
        ...
        loss_tr_list.append(train_loss.numpy())
...
training = True
Model = custom_fit(Model, Tr_sequencer, EPOCHS, Va_sequencer, callbacks=[tensorboard_callback,earlystop])
#train

# Model.fit(Tr_sequencer, epochs=EPOCHS, validation_data =  Va_sequencer, callbacks=[tensorboard_callback,earlystop], verbose = 2)
...
print("Test Loss: ", Test_loss)
    # np.savez_compressed("/content/gdrive/MyDrive/MolecularGNN-main"+str(i)+".npz", result=result)
    # np.savez_compressed("result/"+name+"/"+str(i)+".npz", result=result)

# graphs

graph_name = "Original_model_epoch_100"
...
df["Training Accuracy"] = accuracy_tr_list

df["Training Loss"] = loss_tr_list
df["Validation Accuracy"] = accuracy_val_list
df["Validation Loss"] = loss_val_list

df.to_excel(graph_name +"_results.xlsx")

ax[0].plot(accuracy_tr_list, epoch_list, color='orange')
